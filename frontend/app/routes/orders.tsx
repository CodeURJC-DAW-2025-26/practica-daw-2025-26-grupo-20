...
export async function clientLoader({ request }: { request: Request }) {
  ...
  const response = await fetch(`${API_BASE_URL}/api/v1/orders?page=${page}&size=10`, { credentials: "include" });
  ...
  if (!orders || orders.content?.length === 0) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-4 bg-stone-50 animate-fade-in">
        ...
        <Link to="/menu" className="bg-amber-800 text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-amber-900 transition-all shadow-xl">Empezar a Comprar</Link>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
        <div className="space-y-2">
          ...
          <p className="text-stone-400 font-black text-xs uppercase tracking-[0.3em] flex items-center gap-3">
            <i className="fas fa-history text-amber-800"></i>Tus experiencias en Mokaf
          </p>
        </div>
              ...
              <div className="space-y-6 flex-grow">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                  <div className="space-y-1">
                    ...
                    <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${order.status === 'PAID' ? 'bg-green-100 text-green-700' : order.status === 'CART' ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-700'}`}>{order.status}</span>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] font-black uppercase tracking-widest text-stone-300">Sucursal</p>
            ...
            <div className="mt-10 pt-10 border-t border-stone-50 flex flex-wrap gap-4">
              {order.items?.slice(0, 5).map((item: any) => (
                <div key={item.id} className="relative">
                  <img src={item.productImageUrl ? `${API_BASE_URL}${item.productImageUrl}` : "https://images.unsplash.com/photo-1511920170033-f8396924c348?w=100"} alt="Product" className="w-12 h-12 rounded-xl object-cover border-2 border-white shadow-md" />
                  <div className="absolute -top-2 -right-2 bg-amber-800 text-white w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold">{item.quantity}</div>
                </div>
              ))}
              {order.items?.length > 5 && (
                <div className="w-12 h-12 rounded-xl bg-stone-50 border border-stone-100 flex items-center justify-center text-[10px] font-black text-stone-400">+{order.items.length - 5}</div>
              )}
            </div>
          </div>
        ))}
