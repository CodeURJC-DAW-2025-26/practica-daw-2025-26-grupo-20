...
export default function Statistics() {
  ...
  useEffect(() => {
    if (isUnauthorized || !isLogged) {
      navigate("/login");
    } else if (isForbidden || user?.role !== 'ADMIN') {
      navigate("/menu");
    }
    ...
    <div className="container mx-auto px-4 py-20 max-w-7xl animate-fade-in space-y-16">
      <div className="flex flex-col md:flex-row justify-between items-end gap-8 border-b-2 border-stone-100 pb-12">
        <div className="space-y-4">
           ...
           <p className="text-stone-400 font-bold text-xs uppercase tracking-widest leading-relaxed max-w-lg">Análisis detallado de ventas, rendimiento de sucursales y preferencias de nuestros clientes en tiempo real.</p>
        </div>
        <div className="flex gap-4">
           <div className="bg-stone-900 text-white p-6 rounded-3xl shadow-xl flex items-center gap-6">
              <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-amber-500">
                 <i className="fas fa-chart-line text-xl"></i>
              </div>
              <div>
                 <p className="text-[10px] font-black uppercase tracking-widest text-stone-500">Estado del Sistema</p>
                 <p className="font-black uppercase tracking-tight text-xs">Optimizado</p>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-10">
        {/* Top Product Card */}
        <div className="lg:col-span-2 group bg-white rounded-[3rem] p-12 border border-stone-100 shadow-xl shadow-stone-200/40 relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
           <div className="absolute top-0 right-0 w-64 h-64 bg-amber-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 opacity-50 group-hover:opacity-100 transition-opacity"></div>
           ...
           <div className="flex-grow space-y-6 relative z-10 text-center md:text-left">
              ...
              <div className="grid grid-cols-2 gap-8 pt-4">
                 ...
                 <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-amber-500 font-black text-xl justify-center md:justify-start italic">
                       {stats.bestProduct?.averageRating || 4.9} <i className="fas fa-star text-sm"></i>
                    </div>
                 </div>
              </div>
              <p className="text-stone-400 text-sm font-medium italic border-t border-stone-50 pt-6">"Este producto representa el 24% del volumen de ventas total de este mes."</p>
           </div>
        </div>

        {/* Top Category Mini Card */}
        <div className="bg-stone-900 rounded-[3rem] p-12 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between group">
           <i className="fas fa-mug-hot absolute -bottom-10 -right-10 text-[200px] opacity-[0.03] group-hover:opacity-[0.07] transition-opacity"></i>
           <div className="space-y-8 relative z-10">
              <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center text-amber-500 text-2xl shadow-inner">
           ...
           <div className="mt-12 pt-12 border-t border-stone-800 relative z-10 flex justify-between items-end">
              ...
              <div className="w-12 h-12 rounded-full border-2 border-stone-800 flex items-center justify-center">
                 <i className="fas fa-arrow-trend-up text-amber-500"></i>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-10">
         {/* Branch Ranking */}
         <div className="bg-white rounded-[3rem] p-12 border border-stone-100 shadow-xl shadow-stone-200/20">
            <h3 className="text-2xl font-black text-stone-800 uppercase tracking-tight mb-10 flex items-center gap-4">
               <i className="fas fa-store text-amber-800"></i>
               Rendimiento por Sucursal
            </h3>
            <div className="space-y-8">
               {stats.allBranches?.map((branch: any, idx: number) => (
                  <div key={branch.name} className="flex items-center gap-6 group">
                     <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs ${idx === 0 ? 'bg-amber-800 text-white shadow-lg' : 'bg-stone-50 text-stone-400'}`}>
                        {idx + 1}
                     </div>
                     <div className="flex-grow space-y-2">
                        ...
                        <div className="h-2 bg-stone-50 rounded-full overflow-hidden border border-stone-100 shadow-inner">
                           <div 
                             className="h-full bg-amber-800 rounded-full transition-all duration-1000 shadow-[0_0_20px_rgba(154,103,44,0.4)]" 
                             style={{ width: `${(branch.totalSales / (stats.allBranches[0]?.totalSales || 1)) * 100}%` }}
                           ></div>
                        </div>
                     </div>
                  </div>
         ...
         <div className="bg-stone-50 rounded-[3rem] p-12 border border-stone-100">
            <h3 className="text-2xl font-black text-stone-800 uppercase tracking-tight mb-10 flex items-center gap-4">
               <i className="fas fa-pie-chart text-amber-800"></i>
